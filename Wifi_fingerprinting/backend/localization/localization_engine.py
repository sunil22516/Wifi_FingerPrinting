...
class LocalizationEngine:
    ...
    def __init__(self, radio_map_path: str = None):
        ...
        Parameters:
            radio_map_path: Path to radio_map.json file (built from fingerprints_Awing.csv)
        """
        ...
        raw_fingerprints = data["fingerprints"]
        self.metadata = data.get("metadata", {})
        self.k = 3
        self.weighted = True
        self.last_position = None

        # Normalise fingerprints: wifi_readings values may be either
        #   int  (raw RSSI)  or  dict {"rssi": int, "num_samples": int}
        # knn.py expects {bssid: int} so we flatten here once at load time.
        self.radio_map = []
        for fp in raw_fingerprints:
            flat_readings: Dict[str, int] = {}
            for bssid, val in fp.get("wifi_readings", {}).items():
                if isinstance(val, dict):
                    flat_readings[bssid.lower()] = int(val["rssi"])
                else:
                    flat_readings[bssid.lower()] = int(val)
            self.radio_map.append({
                "x": fp["x"],
                "y": fp["y"],
                "label": fp.get("label", ""),
                "location_id": fp.get("location_id", ""),
                "wifi_readings": flat_readings
            })
